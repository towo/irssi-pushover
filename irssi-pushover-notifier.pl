...
sub send_to_pushover {
	...
	if ($response->is_error()) {
		...
		Irssi::set_settings_bool('pushover_enable', 0);
	}
}
...
sub event_handler {
	...
	if (Irssi::get_settings_bool('pushover_enable')) {
		if (&sanity_check) {
			if ($target) {
				# public message
				send_to_pushover('public', $target, "<$nick> $message");
			} else {
				# private message
				send_to_pushover('private', $nick, $message);
			}
		}
	}
}
...
Irssi::settings_add_bool('', 'pushover_enable', 0);
Irssi::settings_add_str('', 'pushover_api_token', 'iV2kqvQaDnUEOK4UIIT9HtXnL0RtGL');
Irssi::settings_add_str('', 'pushover_target_user', '');
